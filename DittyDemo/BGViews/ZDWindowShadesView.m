...
- (void)setupUI {
    ...
    NSParameterAssert(self.layer.sublayers.count == 0);
    
    if (self.subLayerArray.count > 0) {
        [self.subLayerArray removeAllObjects];
    }
...
- (void)startAnimation:(ShowDirection)direction {
    ...
    switch (direction) {
        case ShowDirection_RightToLeft:
        {
            ...
            [self.subLayerArray enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(CALayer * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                POPBasicAnimation *animation1 = [self animationWithDuration:duration];
                animation1.beginTime = CACurrentMediaTime() + startInterval * (count - 1 - idx);
        ...
        {
            [self middleToSideAnimationWithDuration:duration
        ...
        {
            NSUInteger count = self.subLayerArray.count;
            [self.subLayerArray enumerateObjectsUsingBlock:^(CALayer * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                POPBasicAnimation *animation1 = [self animationWithDuration:duration];
                animation1.beginTime = CACurrentMediaTime() + startInterval * idx;
                animation1.completionBlock = ^(POPAnimation *anim, BOOL finished) {
                    if (finished && (idx == (count - 1))) {
                        [self removeFromSuperview];
                    }
                };
                [obj pop_addAnimation:animation1 forKey:AnimationKey];
...
- (void)middleToSideAnimationWithDuration:(NSTimeInterval)duration startInterval:(NSTimeInterval)startInterval {
    NSMutableArray *leftArray = @[].mutableCopy;
    NSMutableArray *rightArray = @[].mutableCopy;
    
    NSUInteger middle = self.subLayerArray.count / 2; // 中间值
    [self.subLayerArray enumerateObjectsUsingBlock:^(CALayer * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (idx < middle) {
    ...
    NSUInteger maxCount = MAX(leftArray.count, rightArray.count);
    ...
    [rightArray enumerateObjectsUsingBlock:^(CALayer * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        POPBasicAnimation *animation = [self animationWithDuration:duration];
        animation.beginTime = CACurrentMediaTime() + startInterval * idx;
        animation.completionBlock = ^(POPAnimation *anim, BOOL finished) {
            if (finished && (idx == (maxCount - 1))) {
...
- (void)stopAnimation {
    [self.subLayerArray makeObjectsPerformSelector:@selector(pop_removeAnimationForKey:) withObject:AnimationKey];
}
