...
@property (nonatomic, strong) NSMutableArray<NSArray<CAShapeLayer *> *> *dotLineShapeArray;///< 装有点矩阵的二维数组，每一条线上的所有点也是一个数组
...
- (void)addShapePath {
    ...
    rightTop.path = [self lineBezierPathFrom:ZD_RightTopPoint(rightTop.bounds) to:ZD_LeftBottomPoint(rightTop.bounds)].CGPath;
    
    // 左下角
    CAShapeLayer *leftBottom = [self shapeLayerWithFrame:(CGRect){0, selfHeight * aRatio2, shapeLSize}];
    leftBottom.path = [self lineBezierPathFrom:ZD_LeftBottomPoint(leftBottom.bounds) to:ZD_RightTopPoint(leftBottom.bounds)].CGPath;
    
    // 右下角
    CAShapeLayer *rightBottom = [self shapeLayerWithFrame:(CGRect){selfWidth * aRatio2, selfHeight * aRatio2, shapeLSize}];
    rightBottom.path = [self lineBezierPathFrom:ZD_RightBottomPoint(rightBottom.bounds) to:ZD_LeftTopPoint(rightBottom.bounds)].CGPath;
...
- (void)setupObserver {
    ...
    // KVO
    [leftTopShape addObserver:self forKeyPath:KeyPath options:NSKeyValueObservingOptionNew context:nil];
}
...
- (void)startAnimation {
    ...
    void(^displayDotBlock)() = ^() {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(averageInterval * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            self.hollowRectangleLayer.hidden = YES;
            self.solidRectangleLayer.hidden = NO;
        });
        
        // 隐藏点点
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(averageInterval * 2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self displayDotLine:DotLineDisplayType_HideAll];
        });
        
        // 实心矩形变色
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(averageInterval * 3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        // 点点上下边出来
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(averageInterval * 4 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self displayDotLine:DotLineDisplayType_HidelMiddleLines dotColor:self.solidRectangleLayer.fillColor];
        });
        
        // 实心矩形消失
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(averageInterval * 5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            self.solidRectangleLayer.hidden = YES;
        });
        
        // 最后点点消失
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(averageInterval * 6 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self displayDotLine:DotLineDisplayType_HideAll];
    ...
    [self.diagonalShapeArray enumerateObjectsUsingBlock:^(CAShapeLayer * _Nonnull layer, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)hollowRectangle {
    CGPoint point = ZD_PointScale(ZD_RightBottomPoint(self.diagonalShapeArray.firstObject.frame), kScale);
...
- (void)dotRectangle {
    ...
    NSMutableArray<NSArray<CAShapeLayer *> *> *dotLineShapeArray = @[].mutableCopy;     // 二维数组,元素为每一行的点数组
    for (NSUInteger i = 0; i < lineCount; i++) {
        ...
        for (NSUInteger j = 0; j < dotCountInLine; j++) {
            @autoreleasepool {
                ...
                CGPoint center = (CGPoint){dashLine.frame.size.width / 2.0, dashLine.frame.size.height / 2.0};
                dashLine.path = [UIBezierPath bezierPathWithArcCenter:center radius:circleRadius startAngle:0 endAngle:(M_PI * 2) clockwise:YES].CGPath;
...
- (void)displayDotLine:(DotLineDisplayType)type dotColor:(CGColorRef)dotColor {
    [self.dotLineShapeArray enumerateObjectsUsingBlock:^(NSArray<CAShapeLayer *> * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        switch (type) {
...
- (void)dotRectangleXXXX __unused {
    ...
    for (NSUInteger i = 0; i < 4; i++) {
        CAShapeLayer *dashLine = [self shapeLayerWithFrame:(CGRect){x, y + lineMargin * i, width, height}];
        dashLine.lineWidth = customLineWidth;
        dashLine.path = ({
            UIBezierPath *path = [UIBezierPath bezierPath];
            [path moveToPoint:CGPointZero];
            [path addLineToPoint:(CGPoint){CGRectGetMaxX(dashLine.bounds), CGRectGetMinY(dashLine.bounds)}];
            path;
        }).CGPath;
